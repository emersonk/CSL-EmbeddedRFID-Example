...
void check_duplication(CS203DC_Handle* handle, int epcbytelen, unsigned char* curinbuf)
{
	...
	#else
	{
		...
		{
			if(handle->tagbeginidx >= MAX_TAGBUFLEN) break;
			if(epcbytelen == handle->tag[i].len)
			{
				if(memcmp(&curinbuf[2], handle->tag[i].epc, epcbytelen) == 0)
				{
...
int CS203DC_DecodePacket(CS203DC_Handle* handle, unsigned char* inbuf, int inlen, int* inprocessed, unsigned char* outbuf, int outlen, int* outused, unsigned int* errorcode)
{
	...
	*curoutbuf = 0;
	...
	for (k = 0; k < curinlen; k++) sprintf(buf_debugMsg + 2 * k, "%02X", curinbuf[k]);

	if (curinlen < 8)	printf("Error with inlen = %d < 8: %s\n", curinlen, buf_debugMsg);
	while(curinlen >= 8)
	{
				...
				switch(header.pkt_type)
				{
					...
					if((header.flags & 0x01) == 0 && curinbuf[0] == 4)
					{
						...
						do {
							...
							sprintf (curoutbuf, "<TAG="); curoutbuf += 5; curoutlen -= 5; nused += 5;

							sprintf(curoutbuf, "%02X", curinbuf[i++]); sprintf(curoutbuf+2, "%02X,", curinbuf[i++]); curoutbuf += 5; curoutlen -= 5; nused += 5;
					...
					else if((header.flags & 0x01) == 0 && curinbuf[0] == 3)
					{
						...
						if ((20 + 2 + epcbytelen + curinbuf[16] + curinbuf[17] + 2) > packetlen) printf("Error with invalid length epc=%d, data1=%d, data2=%d: inbuf = %s\n", epcbytelen, curinbuf[16], curinbuf[17], buf_debugMsg);
						//printf("v3 inbuf = %s\n", buf_debugMsg);
						...
						if(tag == NULL)
						{
							...
							sprintf(curoutbuf, "%02X", curinbuf[20]); sprintf(curoutbuf+2, "%02X,", curinbuf[21]); curoutbuf += 5; curoutlen -= 5; nused += 5;
							...
							else
							{
								...
								sprintf(curoutbuf, ","); curoutbuf += 1; curoutlen -= 1; nused += 1;
								sprintf(curoutbuf, "%02X", curinbuf[22+i]); sprintf(curoutbuf+2, "%02X", curinbuf[23+i]); curoutbuf += 4; curoutlen -= 4; nused += 4;
							}
...
/////////////////////////////////////////////////////////////////////
					...
					else if((header.flags & 0x01) == 0) printf("Error with non-zero flag: inbuf = %s\n", buf_debugMsg);
					else printf("Error with package version: inbuf = %s\n", buf_debugMsg);
					break;
					...
					case 0x0002:
						...
						break;
					case 0x0007:
						if (handle->simpledisplay == 0) printf ( "--Antenna-Cycle-End: %s\n", buf_debugMsg);
						break;
					case 0x0003:
...
*/		
	...
	if(ret == 0)
	{
		...
		if (0)
		{
			int x;
			printf("OEM data: (");
			for (x=0; x<40; x++) printf("%02X", buf2[x]); printf(")\n");
		}
		if(nrecv)
		{
			int x;
			for (x=0; x<4; x++) handle->OEM_countryversion[x] = buf2[31-x];
	...
	if(ret == 0)
	{
		...
		if (handle->OEM_frequencyModification == 0xAA) printf(": No Frequency Modification (country setting) is allowed in this model.");
		printf("\n");
		switch(handle->OEM_country)
		{
			case 1:
			...
			case 8:
			case 9:
				if (handle->OEM_frequencyModification == 0xAA) break;
...
//	char toSerialBuff[1600];
	...
	if ((toc - handle->tic) != 0)
	{
		...
		handle->tic = toc;
		handle->numTag = 0; handle->newTag = 0; 
	}
...
/*
		if(nprocessed == 36)
			...
			printf ("\n nprocessed : %d \n", nprocessed);
*/
		if (*toSerialBuff) if (handle->simpledisplay == 0)
			printf ("%s\n", toSerialBuff);
...
int config_read0 (char *linebuf, int isFileLine, CS203DC_Handle *CS203fd)
{
	...
	*filename = 0;
	...
	if (isFileLine == 0) printf("Argument"); else printf("Config"); printf (" read1 :");

	spacepos = strchr (linebuf, '-'); if (spacepos != NULL) spacepos--;
	while (spacepos != NULL)
	{
		spacepos ++;
		if ((isFileLine == 0) && (memcmp (spacepos, "-conf", 5) == 0))
		{
			...
			strncpy (CS203fd->configFile, spacepos, 50 - 1); printf("(%s)", CS203fd->configFile);
			break;
		}
...
//new parameters
			...
			spacepos = strchr (spacepos, ' ');
			spacepos ++;			
			if (memcmp (spacepos, "normal", 6) == 0) CS203fd->inventorymode = 0;
			else if (memcmp (spacepos, "compact", 7) == 0) CS203fd->inventorymode = 1;
...
int conf_Init (char *confFile, CS203DC_Handle *CS203fd_first)
{
	...
	while (fscanf (fin, "%[^\n]\n", linebuf0) != EOF)
	{
		...
		strncpy(linebuf + linebuf_index, linebuf0, strlen(linebuf0)); linebuf_index += strlen(linebuf0);
	}
	linebuf[linebuf_index] = 0;
	{
		...
		{
			if ((linebuf[x] != ' ') || ((linebuf1[y-1] != ' ') && y != 0)) linebuf1[y++] = linebuf[x];  
		}
		printf ("Conf input : %s\n", linebuf1);
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	if (handle->inventorymode) t_inv_mode = 0x04;
...
//new parameters
				...
				if (strcmp (&av[cnt+1][0], "normal") == 0) CS203fd->inventorymode = 0;
				else if (strcmp (&av[cnt+1][0], "compact") == 0) CS203fd->inventorymode = 1;
...
//new parameters
	...
	} else conf_Init (CS203fd_first->configFile, CS203fd_first);
